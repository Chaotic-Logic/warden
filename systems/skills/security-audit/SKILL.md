---
name: security-audit
description: Security posture and hardening audit for a Linux host — the Security Report half of a warden sweep. Covers a hardening scan (Lynis, OpenSCAP/oscap against a CIS or STIG profile), attack-surface review (listening ports, firewall state, exposed services), account and auth hygiene (SSH config, sudoers, failed logins, stale/empty-password accounts), filesystem risk (SUID/SGID, world-writable, package integrity), rootkit checks, and matching installed packages against known CVEs using the distro's own security tooling. Read-only and non-destructive; remediation is proposed, shown, and confirmed separately. Use for auditing a running Linux host's security: "run a security scan on my server", "audit my server's security", "check for CVEs / vulnerable packages", "is this box hardened", "harden this server", or the security half of a health check. This audits a live host's posture, not source code — for reviewing code, a diff, or a repo for vulnerabilities, use code review, not this. Triggers on host security work: "security scan/audit a server", "harden a host", "CVE / vulnerable packages on the box", "open ports audit", "is my server secure", "compliance/CIS scan", "lynis", "openscap". Not for auditing application source or a code diff.
---
...
**6. CVEs vs installed packages** — the real "am I running something known-bad" check. Distro-specific; the commands are in `references/cve-by-distro.md`. In short: use the distro's own security-update feed first (`dnf updateinfo list security`, `apt` + `debsecan`, `arch-audit`, `zypper list-patches`), because it maps CVEs to the exact patched package version for your release. Reach for Grype/Trivy on top when you want a scanner's view or you're auditing container images. And if the OS itself is end-of-life (CentOS 7, an EOL Ubuntu, anything past its support window), that outranks any single CVE: no security feed means the box only rots from here. The call is regenerate, not resurrect — a clean rebuild on the current release of the same family (Rocky/Alma/Fedora, Debian/Ubuntu LTS, or openSUSE Leap/SLES; rolling distros catch up with a full `-Syu` or get reinstalled) over an in-place major-version upgrade that drags the old cruft and old mistakes forward. But that's a recommendation, not a refusal to help: if they can't rebuild right now (and often they can't), still harden the box they have as far as it'll go — cut attack surface (drop unneeded packages/services), lock the firewall to mesh-only management, get the MAC enforcing, keep fail2ban up, tighten monitoring, and check for an extended-support channel that still ships CVE fixes for this release (Ubuntu Pro/ESM, Debian ELTS, TuxCare or vendor ELS for RHEL/CentOS). Be straight that none of this makes an EOL box *safe*; it shrinks the blast radius while the rebuild gets planned.
