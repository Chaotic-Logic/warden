---
name: triage
description: Diagnose and run down a specific fault on a Linux host — a service that won't start or keeps crashing, a process hanging or eating resources, a box gone slow, disk-full mysteries, OOM kills, port conflicts, failed boots. Method-driven root-cause work: reproduce, read the logs, form a hypothesis, isolate, then propose the smallest fix. Diagnosis runs in loose/fast mode (the vibe-gate allows it); the moment a fix changes state it flips to structured and gets confirmed. Use when a Linux host or its services are actually broken and need figuring out, not a routine sweep. This is for operating-system and server faults, not application-code bugs, failing tests, or build/CI errors — if the fault is in a program's own logic or a repo's tooling, this is the wrong skill. Triggers on OS/server faults: "service won't start", "systemd unit keeps crashing", "why is my server/box slow", "process hung", "OOM kill", "disk full", "port already in use", "failed to start", "journalctl", "coredump on the host". Not for debugging application source, test failures, or CI pipelines.
---
